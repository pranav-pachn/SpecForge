...
export type TabId = 'spec' | 'clarify' | 'plan' | 'tasks' | 'execute' | 'review' | 'engineering_review' | 'validate' | 'drift';
...
export default function WorkflowTabs({ activeTab, onTabChange, workflow }: WorkflowTabsProps) {
  ...
  return (
    <div 
      ...
      className="flex items-center gap-1 overflow-x-auto border-b border-white/10 glass p-1 mb-6 scrollbar-hide relative rounded-xl"
    >
      {TABS.map((tab) => {
        const Icon = tab.icon;
        ...
        return (
          ...
          >
            ...
            {isActive && (
              <span className="absolute bottom-0 left-0 right-0 h-0.5 bg-blue-500 rounded-t-full transition-all duration-300" />
            )}
